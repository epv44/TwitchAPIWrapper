...
class VideoModelTests: XCTestCase {
    func testBuildVideoModelFromJSON() {
        ...
        do {
            ...
            let date = ISO8601DateFormatter().date(from: "2018-03-02T20:53:41Z")
            ...
            XCTAssertEqual(video.thumbnailURL.absoluteString, URL(string: "https://static-cdn.jtvnw.net/s3_vods/bebc8cba2926d1967418_chewiemelodies_27786761696_805342775/thumb/thumb0-%{width}x%{height}.jpg")?.absoluteString)
            XCTAssertEqual(video.title, "-")
            XCTAssertEqual(video.type, VideoType.archive)
            XCTAssertEqual(video.url.absoluteString, URL(string: "https://www.twitch.tv/videos/234482848")?.absoluteString)
